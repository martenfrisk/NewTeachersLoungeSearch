<script lang="ts">
	...
	let { message, type = 'info', duration = 5000, show = true, onClose }: Props = $props();
	...
	let toastClasses = $derived(() => {
		const baseClasses =
			'fixed top-4 right-4 z-50 max-w-md p-4 rounded-lg shadow-lg border flex items-center gap-3';
		switch (type) {
			case 'success':
...
{#if show}
	<div class={toastClasses()} transition:fade={{ duration: 200 }} role="alert" aria-live="polite">
